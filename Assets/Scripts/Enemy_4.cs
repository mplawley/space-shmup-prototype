...
{
	//Enemy_4 is a boss enemy of sorts. It will start offscreen and then pick a random point on screen to move to.
	//Once it has arrived, it will pick another random point and continue until the player has shot it down.
	...
	void Start()
	{
		...
		//There is already an initial position chosen by Main.SpawnEnemy(), so add it to points as the initial p0 and p1
		points[0] = pos;
		points[1] = pos;
	...
	//This will override the OnCollisionEnter that is part of Enemy.cs. Because of the way that MonoBehavior
	...
	void OnCollisionEnter(Collision coll)
	{
		...
		case "ProjectileHero":
			...
			//Enemies don't take damage unless they're on screen. This stops the player
			//from shooting them before they are visible.
			...
			//Because there was a collision, we're guaranteed that there is at least a contacts[0], and
			//ContactsPoints have a reference to thisCollider, which will be the collider for the part of the Enemey_4
			//that was hit.
			...
			if (prtHit.protectedBy != null)
			{
				foreach (string s in prtHit.protectedBy)
				{
					...
					if (!Destroyed(s))
					{
						...
						Destroy(other); //Destroy the ProjetileHero
						return; //return before causing damage
					}
				}
			}

			//It's not protected, so make it take damage. Get the damage amount from the Projectile.type & Main.W_DEFS
			...
			if (prtHit.health <= 0)
			{
				//Instead of Destroying this enemey, disable the damaged part
